...
bool pathExists(char maze[][10], int sr, int sc, int er, int ec)
{
	...
	coordQueue.push(startingCoord); // push the starting coordinate on the stack
	...
	{
		Coord currentCoord = coordQueue.front(); // keep track of the current coordinate
		coordQueue.pop(); // pop the current coordinate off of the stack
		...
		if (maze[row - 1][col] != 'X' && maze[row - 1][col] != '@') // can move north & not visited?
		{
			...
			coordQueue.push(currentCoord); // push the current coordinate onto the stack
		...
		if (maze[row][col - 1] != 'X' && maze[row][col - 1] != '@') // can move west & not visited?
		{
			...
			coordQueue.push(currentCoord); // push the coordinate onto the stack
